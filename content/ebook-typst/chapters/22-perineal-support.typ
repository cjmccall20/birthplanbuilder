#import "../template.typ": *
= Chapter 26: Perineal Support: Preventing and Healing Tears <ch-perineal-support>

== THE QUICK ANSWER

About 85-90% of first-time mothers experience some degree of perineal tearing, but most tears are minor (first or second degree) and heal well. You have real tools to reduce your risk: perineal massage starting at 34-36 weeks, warm compresses during pushing, slow and controlled crowning, and choosing birth positions that are gentler on the perineum (side-lying and hands-and-knees are among the best). Routine episiotomy -- a surgical cut to the perineum -- is now recognized as outdated; natural tears actually heal better than surgical cuts. The exception is a targeted episiotomy to prevent a clitoral tear, which is one of the few strong medical indications for the procedure.

*Bottom line:* Tearing is common but not inevitable, and there is a lot you can do to minimize it. Ask your provider about their episiotomy rate and their approach to perineal support during crowning. If tearing does happen, recovery is manageable with ice, sitz baths, stool softeners, and time.

#line(length: 100%)

During vaginal birth, the tissue between your vagina and rectum (called the perineum) stretches to allow your baby to pass through. Sometimes this tissue tears. While that might sound scary, understanding what happens—and what you can do about it—puts you in control.

This section covers everything you need to know: what tearing actually means, how to reduce your chances of it happening, the truth about episiotomy, and how to heal afterward.

#line(length: 100%)

== UNDERSTANDING TEARING

=== What Is Perineal Tearing?

When your baby's head emerges during birth, your perineum stretches significantly. If it stretches beyond its capacity, a tear can occur. This is one of the most common experiences in vaginal birth, especially for first-time mothers.#citation[1]

=== The Four Degrees of Tears

Doctors classify tears by their depth:

_First-Degree Tear_
- The mildest type
- Only affects the skin of the perineum
- May need a few stitches or none at all
- Usually heals within a couple of weeks
- Causes minimal discomfort

_Second-Degree Tear_
- The most common type
- Affects the skin and the muscle beneath it
- Requires stitches
- Takes about 4-6 weeks to heal #citation[2]
- Causes moderate discomfort that improves daily

_Third-Degree Tear_
- Extends into the muscle around the anus (the anal sphincter)
- Requires repair in an operating room
- Takes 6-8 weeks or longer to heal #citation[3]
- Needs careful follow-up care
- Occurs in about 3-4% of vaginal births #citation[4]

_Fourth-Degree Tear_
- The most severe type
- Extends through the anal sphincter into the rectum
- Requires surgical repair
- Needs specialized follow-up care
- Rare—occurs in less than 1% of vaginal births #citation[4]

=== How Common Is Tearing?

Here's what the research tells us:

- About 85-90% of first-time mothers experience some degree of tearing #citation[5]
- Most tears are first or second degree
- Second and later births have lower tearing rates
- Severe tears (third and fourth degree) happen in less than 5% of births #citation[4]

=== What Affects Your Risk?

Several factors influence whether you'll tear and how severely:

_Factors you can influence:_
- Perineal massage during pregnancy (reduces severe tears)
- Birth position (some positions are gentler on the perineum)
- Speed of pushing (slow and controlled is better)
- Provider techniques during crowning

_Factors outside your control:_
- Baby's size and position
- First birth vs. subsequent births
- Your tissue elasticity
- How quickly labor progresses
- Shoulder dystocia or other complications

_Note:_ Many factors are out of your hands, and that's okay. Tearing doesn't mean you or your provider did something wrong—it's often just part of how birth unfolds.

#line(length: 100%)

== PREVENTION DURING PREGNANCY

=== Perineal Massage

Perineal massage is a technique you can do at home starting around 34-36 weeks of pregnancy.#citation[6] Research shows it can:

- Reduce the chance of severe tears (especially for first-time mothers) #citation[6]
- Decrease the likelihood of needing an episiotomy #citation[6]
- Help you become familiar with the stretching sensation you'll feel during birth

_How to do it:_

1. Wash your hands thoroughly
2. Use a natural lubricant (olive oil, coconut oil, or water-based lubricant)
3. Insert your thumbs about 1-2 inches into your vagina
4. Press downward toward your rectum
5. Hold the stretch for 1-2 minutes—you should feel stretching but not pain
6. Gently massage the tissue in a U-shaped motion
7. Practice for 5-10 minutes, 2-3 times per week

_Tips for success:_
- Many people find it easier to have their partner help
- A warm bath beforehand can relax the tissue
- Stop if you experience pain or bleeding
- This takes practice—it gets easier over time

=== General Fitness

While there's no magic exercise to prevent tearing, staying generally healthy during pregnancy supports tissue elasticity and recovery:

- Stay hydrated (good for tissue health)
- Eat a balanced diet with adequate protein
- Gentle movement and stretching keep your body flexible
- Kegel exercises strengthen your pelvic floor (though this is more about recovery than preventing tears)

#line(length: 100%)

== PREVENTION DURING BIRTH

What happens during labor and delivery has the biggest impact on tearing. Here's what the evidence supports:

=== Warm Compresses

Applying warm compresses to your perineum during pushing is one of the most effective prevention techniques:#citation[7]

- Reduces the rate of severe tears #citation[7]
- Helps the tissue become more flexible
- Feels soothing during an intense time
- No risks or downsides

_Ask your provider or nurse to apply warm compresses during the pushing stage._ This is a simple request that can make a real difference.

=== Slow, Controlled Crowning

How your baby's head emerges matters enormously:

- _Slow is better._ When the head crowns slowly, your tissue has time to stretch.
- _Breathing through crowning._ Instead of forceful pushing when the head is emerging, small breaths or "panting" gives tissue more time.
- _Hands-on support._ Your provider can apply gentle counter-pressure to help control the speed.

_What to ask your provider:_
- "Can you help me slow down when the baby is crowning?"
- "Will you guide me on when to push and when to breathe?"
- "Can we take extra time during crowning if possible?"

=== Positions That Reduce Tearing

Your birth position affects perineal stretching. Positions that tend to be gentler:

_Side-lying_ #citation[8]
- Takes pressure off the perineum
- Allows for slow, controlled delivery
- One of the gentlest positions for your tissue

_Hands and knees_
- Opens the pelvis
- Uses gravity differently than on your back
- Allows for natural positioning of baby's head

_Semi-reclined (not flat on back)_
- Better than flat on your back
- Still allows some gravity assistance
- Common in many birth settings

_Positions that increase tearing risk:_ #citation[8]
- Lying flat on your back with legs in stirrups
- Deep squatting (though this can be helpful for other reasons)
- Lithotomy position (flat on back, legs held up)

_Important:_ The "best" position is one where you feel comfortable and your baby is doing well. Don't sacrifice a safe delivery to avoid tearing—but if you have options, these positions are worth considering.

=== Other Helpful Techniques

_Water birth:_ Some research suggests laboring or delivering in water reduces tearing, possibly because warm water relaxes the perineum.#citation[9]

_Avoid directed pushing:_ When providers count and direct pushing ("Push! Push! Push!"), it can lead to faster, more forceful delivery. Following your body's urges may result in gentler births.#citation[10]

_Allow time between contractions:_ During crowning, pausing between pushes lets tissue stretch gradually.

#line(length: 100%)

== EPISIOTOMY: THE TRUTH

An episiotomy is a surgical cut made to the perineum to widen the vaginal opening during delivery. For decades, this was done routinely. Research has since proven this approach wrong.

=== Routine Episiotomy Is Outdated

Here's what major medical organizations now agree on:

- _Routine episiotomy should NOT be performed._ It does not prevent severe tears—it actually increases the risk.#citation[11]
- _Natural tears heal better than surgical cuts._ Tears follow the natural fiber of the tissue; cuts do not.#citation[11]
- _Recovery is often harder after episiotomy._ More pain, longer healing, higher infection risk.#citation[11]

The American College of Obstetricians and Gynecologists recommends against routine episiotomy.#citation[12] If your provider suggests one "just to help things along," you can ask questions.

=== When Episiotomy Might Actually Be Needed

In rare situations, episiotomy may be medically necessary:

- _Severe fetal distress_ requiring immediate delivery
- _Shoulder dystocia_ where the baby's shoulder is stuck
- _Instrumental delivery_ (forceps or vacuum) where more space is urgently needed
- _When the baby needs to be delivered immediately_ and there isn't time for natural stretching

One of the clearest medical indications for episiotomy is the risk of a clitoral tear. It is possible for a mother to tear upward — toward and into the clitoris — rather than downward toward the perineum. This is extremely painful and serious. If your provider sees during crowning that an upward tear is imminent, they may strongly recommend a controlled episiotomy — a deliberate downward cut — to redirect the tear away from the clitoris. This is a situation where your provider's recommendation deserves serious weight. A clitoral tear can cause significant long-term consequences for sensation, sexual function, and recovery that go well beyond what a typical perineal tear involves. Unlike routine episiotomy, which the evidence clearly argues against, a targeted cut to prevent clitoral damage is one of the strongest reasons the procedure still exists.

Outside of situations like these, episiotomy is uncommon. Most births do not require one.

=== Questions to Ask Your Provider

Before birth:
- "What is your episiotomy rate?"
- "Under what circumstances would you perform one?"
- "Can we discuss before you cut, unless it's an emergency?"

A provider with a low episiotomy rate (under 10-15%) is following evidence-based practice. If a provider routinely performs episiotomies, consider this a red flag.

=== Natural Tears vs. Episiotomy: The Comparison

#table(
  columns: 2,
  [*Natural Tear*],
  [*Episiotomy*],
  [Follows tissue fibers],
  [Cuts across tissue fibers],
  [Usually smaller],
  [Predetermined size],
  [Lower infection risk],
  [Higher infection risk],
  [Faster healing],
  [Slower healing],
  [Less long-term pain],
  [More long-term pain],
  [May be irregular],
  [Clean edge],
)

The "clean edge" of an episiotomy was once thought to be an advantage. Research has proven otherwise—irregular natural tears actually heal better because they follow your body's natural structure.#citation[11]

#line(length: 100%)

== HEALING AFTER BIRTH

No matter what degree of tearing you experience, healing takes time. Here's what to expect and how to cope.

=== What to Expect

_First few days:_
- Significant soreness and swelling
- Difficulty sitting comfortably
- Burning with urination (tip: pour warm water over your perineum while you pee)
- Stitches may feel tight or itchy

_First two weeks:_
- Gradual improvement in pain
- Swelling decreases
- Sitting becomes easier
- Stitches begin to dissolve

_Weeks 2-6:_
- Most first and second-degree tears are healed
- Discomfort becomes minimal
- Normal activities resume
- Stitches fully dissolve

_Third and fourth-degree tears:_
- Healing takes longer (6-12 weeks) #citation[3]
- May need additional follow-up appointments
- Physical therapy may be recommended
- Full recovery is possible but takes patience

=== Pain Management

_Ice packs (first 24-48 hours)_
- Reduce swelling
- Numb the area
- Use a barrier (thin cloth) between ice and skin
- Apply for 20 minutes on, 20 minutes off

_"Padsicles" (frozen pads)_
A popular home remedy:
1. Open a maxi pad
2. Spread aloe vera gel and witch hazel across it
3. Fold it closed and freeze
4. Use as a cold, soothing pad

_Sitz baths_
- Fill a shallow basin or tub with warm water
- Sit for 10-20 minutes, 2-3 times daily
- Adding Epsom salts may help
- Pat (don't rub) dry afterward

_Pain medication_
- Ibuprofen reduces both pain and swelling #citation[13]
- Acetaminophen is safe while breastfeeding #citation[14]
- Prescription options if needed—ask your provider
- Numbing sprays (like Dermoplast) provide quick relief

_Stool softeners_
- Your first bowel movement after tearing can be intimidating
- Stool softeners make this much easier
- Start taking them immediately after birth
- Drink plenty of water and eat fiber

=== Comfort Strategies

_Sitting:_
- Use a donut pillow or inflatable ring
- Sit on a folded towel
- Lean to one side
- Avoid hard surfaces when possible

_Going to the bathroom:_
- Use a peri bottle (squeeze bottle) with warm water while urinating
- Hold a clean pad against your perineum during bowel movements for support
- Don't strain—let the stool softeners do their job
- Pat gently or air dry

_Moving around:_
- Take it slow
- Avoid stairs when possible in the first few days
- Gentle walking is fine and helps healing
- No heavy lifting

_Keeping clean:_
- Use the peri bottle after every bathroom trip
- Pat dry gently
- Change pads frequently
- Avoid baths in the first few days if you have stitches (showers are fine)

=== Long-Term Recovery

Most people recover fully from perineal tears. However, some may experience:

- Scar tissue that feels different
- Discomfort during sex (usually temporary)
- Changed sensations in the area

_Resuming sex:_
- Wait until your provider clears you (usually 6 weeks)
- Use plenty of lubrication
- Go slowly
- Communicate with your partner
- Some discomfort is normal at first—pain is not

If you had a severe tear, pelvic floor physical therapy can make a significant difference in your recovery.#citation[15] Ask your provider for a referral if you experience ongoing issues.

#line(length: 100%)

== WHEN TO CALL YOUR PROVIDER

Healing from a tear is normal, but some symptoms require medical attention. _Call your provider if you experience:_

_Signs of infection:_
- Increasing pain (instead of decreasing)
- Redness or warmth spreading from the wound
- Pus or foul-smelling discharge
- Fever over 100.4°F (38°C)
- Feeling generally unwell

_Wound problems:_
- Stitches opening or coming apart
- Excessive bleeding (soaking a pad in an hour)
- A gap appearing in the wound

_Bowel or bladder issues:_
- Unable to control gas or stool
- Pain with bowel movements that isn't improving
- Unable to urinate or pain that gets worse with urination

_Other concerns:_
- Pain that isn't improving with medication
- Discomfort that interferes with caring for your baby
- Any symptom that worries you

_Trust your instincts._ You know your body. If something doesn't feel right, it's always okay to call.

#line(length: 100%)

== KEY TAKEAWAYS

1. _Tearing is common_—most first-time mothers experience some degree of tear, and most are minor.

2. _Prevention is possible_ through perineal massage, warm compresses, slow crowning, and birth positioning.

3. _Routine episiotomy is outdated_—natural tears heal better than surgical cuts.

4. _Healing takes time_ but is completely manageable with ice, sitz baths, pain relief, and patience.

5. _Severe tears require extra care_ but full recovery is absolutely possible.

6. _Call your provider_ if you see signs of infection, wound problems, or symptoms that worry you.

You may not have complete control over whether you tear, but you have more influence than you might think. Talk to your provider about perineal support, advocate for slow crowning, and know that whatever happens, your body knows how to heal.

#line(length: 100%)

_This guide is for informational purposes only and should not replace advice from your healthcare provider. Every birth is different, and your provider can give you personalized recommendations based on your specific situation._

#line(length: 100%)

== Citations

See #link(<cite-ch-perineal-support>)[all citations for this chapter] in the Citations Appendix.